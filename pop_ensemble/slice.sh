...
OUTFILE=COMP_${VARNAME}_${CASESUF}_${FILESUF}_${DIRECTION}_${VAL}


ThisDir=$( cd `dirname $0`; pwd -P )
NCLSCRIPT=$ThisDir/ncl_library/slice.ncl

export NCL_VALID_LIB_DIR=$( cd $ThisDir/ncl_library; pwd -P )
COMMAND="ncl $NCLSCRIPT testfile=\"${FILE1}\" cntrlfile=\"$FILE2\" varname=\"$VARNAME\" outfile=\"$OUTFILE\" direction=\"$DIRECTION\" val=\"$VAL\" "
echo "$COMMAND"
$COMMAND
