...
!-----------------------------------------------------------------------
   ...
   !if (FirstRunCsi == 0 ) then
       ...
       uvsum(1) = POP_GlobalSum(work1, POP_distrbTropic, &
			        POP_gridHorzLocCenter,   &
				errorCode, mMask = mMaskTropic)
       uvsum(2) = POP_GlobalSum(work0, POP_distrbTropic, &
			        POP_gridHorzLocCenter,   &
				errorCode, mMask = mMaskTropic)
       csv = uvsum(1)/uvsum(2)
   ...
   if (POP_myTask == POP_masterTask) &
     write(*,*) "Init uv: ", uvsum(1),uvsum(2), csu, csv
...
!-----------------------------------------------------------------------
   ...
   if (FirstRunCsi == 0 ) then
	   ...
	   do j=1,ny
      	   do i=1,nx
	      if (btropWgtCenter(i,j,iblock) /= 0.0_POP_r8) then
		    S(i,j,iblock) = S(i,j,iblock)/btropWgtCenter(i,j,iblock)
		   ! work1(i,j,iblock) = work1(i,j,iblock)/btropWgtCenter(i,j,iblock)
	      else 
		    S(i,j,iblock) = 0.0_POP_r8
		   ! work1(i,j,iblock) = 0.0_POP_r8
	      endif
   ...
   call POP_HaloUpdate(S, POP_haloTropic, POP_gridHorzLocCenter, &
                       POP_fieldKindScalar, errorCode)
   ...
   do iblock=1,numBlocks
      ...
      do i=1,nx
         if (btropWgtCenter(i,j,iblock) /= 0.0_POP_r8) then
   	    work1(i,j,iblock) = Q(i,j,iblock)/btropWgtCenter(i,j,iblock)
         else 
   	    work1(i,j,iblock) = 0.0_POP_r8
         endif
   
         X(i,j,iblock) = X(i,j,iblock) + work1(i,j,iblock)
      end do
      end do
