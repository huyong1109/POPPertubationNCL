...
!-----------------------------------------------------------------------
   ...
   data EvpPreFlag /0/
   save histmaxcc
   data histmaxcc /0.0_POP_r8,0.0_POP_r8/
...
!-----------------------------------------------------------------------
   ...
   if (POP_myTask < numProcs) then
     ...
     if (usePreconditioner) then
      maxcc = maxval(abs(btropWgtCenter))
      EvpTimeStep = 0

      do i = 1, 2
        if(maxcc == histmaxcc(i))  EvpTimeStep = i
      end do 

      if (EvpTimeStep == 0) then
        EvpPreFlag = EvpPreFlag +1
        histmaxcc(EvpPreFlag) = maxcc
      ...
      case (solverChoiceAST)  !hy-csi

         call ast(pressTropic, rhsTropic, errorCode)

         if (errorCode /= POP_Success) then
            call POP_ErrorSet(errorCode, &
               'POP_SolverRun: error in AST')
            return
         endif
...
! !INTERFACE:
 ...
 subroutine ast(X,B,errorCode)  !hy-cst

! !DESCRIPTION:
!  This routine uses a AST solver to
!  solve the equation $Ax=b$.  Both the operator $A$ and preconditioner
!  are nine-point stencils. If no preconditioner has been supplied,
!  a diagonal preconditioner is applied.  Convergence is checked
!  every {\em ncheck} steps.
...
!-----------------------------------------------------------------------
   ...
   integer (POP_i4) :: &
      i,j,m,n,in_iter,           &! local iteration counter
      nx, ny,          &! horizontal extents
      numBlocks,       &! number of local blocks
      iblock            ! local block     counter
...
!-----------------------------------------------------------------------
   ...
   if (errorCode /= POP_Success) then
       call POP_ErrorSet(errorCode, &
       'POP_SolversAST: error retrieving local block count')
       return
   endif
   ...
   if (FirstRunCsi == 0 ) then
        ...
        FirstRunCsi = 1
        if (POP_myTask == POP_masterTask) then 
            write(POP_stdout,*) "Estimate eigenvalues: ", scsu, scsv
        end if 

        call lanczos(nx, ny, numBlocks, LanczosStep, scsu, scsv, errorCode)

        if (POP_myTask == POP_masterTask) then 
            write(POP_stdout,*) "Lanczos Step: ", LanczosStep
            write(POP_stdout,*) "Lanczos eigenvalues: ", scsu,scsv
        end if 
   ...
   if (errorCode /= POP_Success) then
        call POP_ErrorSet(errorCode, &
        'POP_SolversAST: error updating initial residual halo')
        return
   endif
   ...
   !-----------------------------------------------------------------------
   m = 1
   iterationLoop: do while (m < maxIterations) 

   csomga = 2.0_POP_r8/csgamma
   !$OMP PARALLEL DO PRIVATE(iblock)
   do iblock=1,numBlocks
        if (usePreconditioner) then
            call preconditioner(work1,R,iblock)
        ...
        do j=1,ny
        do i=1,nx
            Q(i,j,iblock) = (1.0_POP_r8/csgamma)*R(i,j,iblock)
   ...
   if (errorCode /= POP_Success) then
       call POP_ErrorSet(errorCode, &
       'POP_SolversAST: error computing convergence dot prod')
       return
   endif

   if (rr0 < convergenceCriterion) then
       numIterations = m
       exit iterationLoop
   endif


   exp_iter = acosh(rr0/convergenceCriterion)/acosh(csbeta)
   in_iter = min(maxIterations-m, max(5, floor(exp_iter/2.0_POP_r8)))

   inner_loop: do n = 1, in_iter
   ...
   do iblock=1,numBlocks
        ...
        do i=1,nx
            R(i,j,iblock) = R(i,j,iblock)*A0R(i,j,iblock)
            Q(i,j,iblock) = csomga*R(i,j,iblock)+ (csgamma*csomga-1.0_POP_r8)*Q(i,j,iblock)
            X(i,j,iblock) = X(i,j,iblock) + Q(i,j,iblock)
        end do 
        end do
   ...
   elseif (m < maxIterations ) then 
       ...
       m = m+1
       csc =rr0/rr
       if(csc > 1.0_POP_r8) then 
           csv1 = csbeta*(1.0_POP_r8 -1.0_POP_r8/csc)/csalpha - &
           (csc*csc-1.0_POP_r8)*cheb(j, csbeta)/cheb(j+1,csbeta)/2.0_POP_r8/csalpha/csc
           if(csv1 < 0 ) then
               csv = csv1
           endif 
       endif 
   ...
   if (numIterations == maxIterations) then
       if (convergenceCriterion /= 0.0_POP_r8) then
           call POP_ErrorSet(errorCode, &
           'POP_SolversAST: solver not converged')
           return
       endif
   endif
...
!-----------------------------------------------------------------------
   ...
   if (FirstRunCsi == 0 ) then
      ...
      if (POP_myTask == POP_masterTask) then 
          write(POP_stdout,'(a20,2f15.5)') "Estimate eigenvalues: ", csu, csv
      end if 

      call lanczos(nx, ny, numBlocks,LanczosStep, csu, csv, errorCode)

      if (POP_myTask == POP_masterTask) then 
          write(POP_stdout,*) "Lanczos Step: ", LanczosStep
          write(POP_stdout,'(a20,2f15.5)') "Lanczos eigenvalues: ", csu,csv
      end if 
      !call POP_ErrorSet(errorCode, &
...
!***********************************************************************
!BOP
! !IROUTINE: btropOperatorAbs
! !INTERFACE:

 subroutine btropOperatorAbs(AX,X,bid)
...
!-----------------------------------------------------------------------
   ...
   do j=2,size(X,dim=2)-1
   do i=2,size(X,dim=1)-1
      AX(i,j,bid) =abs( btropWgtCenter(i  ,j  ,bid)*X(i  ,j  ,bid)) + &
                   abs( btropWgtNorth (i  ,j  ,bid)*X(i  ,j+1,bid)) + &
                   abs( btropWgtNorth (i  ,j-1,bid)*X(i  ,j-1,bid)) + &
                   abs( btropWgtEast  (i  ,j  ,bid)*X(i+1,j  ,bid)) + &
                   abs( btropWgtEast  (i-1,j  ,bid)*X(i-1,j  ,bid)) + &
                   abs( btropWgtNE    (i  ,j  ,bid)*X(i+1,j+1,bid)) + &
                   abs( btropWgtNE    (i  ,j-1,bid)*X(i+1,j-1,bid)) + &
                   abs( btropWgtNE    (i-1,j  ,bid)*X(i-1,j+1,bid)) + &
                   abs( btropWgtNE    (i-1,j-1,bid)*X(i-1,j-1,bid))
   end do
   end do
